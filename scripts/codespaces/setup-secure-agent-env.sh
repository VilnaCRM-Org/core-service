...
# Safer default for Claude Code tool permissions.
: "${CLAUDE_PERMISSION_MODE:=plan}"
: "${CLAUDE_ALLOW_BYPASS_PERMISSIONS:=0}"
...
persist_agent_secrets_file() {
...

    {
        ...
        printf 'export CLAUDE_ALLOW_BYPASS_PERMISSIONS=%q\n' "${CLAUDE_ALLOW_BYPASS_PERMISSIONS}"
...
write_codex_config() {
    local tmp_config
    local profile_name_escaped
    local model_escaped
    local reasoning_effort_escaped
    local reasoning_summary_escaped
    local approval_policy_escaped
    local sandbox_mode_escaped
    local openrouter_base_url_escaped

    tmp_config="$(mktemp)"
    track_tmp_file "${tmp_config}"

    profile_name_escaped="$(escape_toml_string "${CODEX_PROFILE_NAME}")"
    model_escaped="$(escape_toml_string "${CODEX_MODEL}")"
    reasoning_effort_escaped="$(escape_toml_string "${CODEX_REASONING_EFFORT}")"
    reasoning_summary_escaped="$(escape_toml_string "${CODEX_REASONING_SUMMARY}")"
    approval_policy_escaped="$(escape_toml_string "${CODEX_APPROVAL_POLICY}")"
    sandbox_mode_escaped="$(escape_toml_string "${CODEX_SANDBOX_MODE}")"
    openrouter_base_url_escaped="$(escape_toml_string "${CODEX_OPENROUTER_BASE_URL}")"

    cat > "${tmp_config}" <<EOM
# Generated by scripts/codespaces/setup-secure-agent-env.sh
profile = "${profile_name_escaped}"

[profiles."${profile_name_escaped}"]
model = "${model_escaped}"
model_provider = "openrouter"
model_reasoning_effort = "${reasoning_effort_escaped}"
model_reasoning_summary = "${reasoning_summary_escaped}"
approval_policy = "${approval_policy_escaped}"
sandbox_mode = "${sandbox_mode_escaped}"

[model_providers.openrouter]
name = "OpenRouter"
base_url = "${openrouter_base_url_escaped}"
...
validate_claude_safety_settings() {
    if [ "${CLAUDE_PERMISSION_MODE}" = "bypassPermissions" ] \
        && [ "${CLAUDE_ALLOW_BYPASS_PERMISSIONS}" != "1" ]; then
        cat >&2 <<'EOM'
Error: refusing unsafe Claude default permission mode.
Detected:
  CLAUDE_PERMISSION_MODE=bypassPermissions

Set CLAUDE_ALLOW_BYPASS_PERMISSIONS=1 only when this is an intentional opt-in.
EOM
        exit 1
    fi
}
...
cs_warn_if_missing_command codex || true
