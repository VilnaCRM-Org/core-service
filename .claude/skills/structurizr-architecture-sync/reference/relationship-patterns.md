...
customerController -> commandBus "uses"
...
customerController -> commandBus "uses to dispatch commands"
...
customerCreatedEvent -> emailSubscriber "triggers"
customerCreatedEvent -> analyticsSubscriber "triggers"
customerCreatedEvent -> notificationSubscriber "triggers"
```
...
customer -> customerUpdatedEvent "publishes"
customer -> customerDeletedEvent "publishes"
```
...
# Combined (common)
customerRepository -> database "persists to / retrieves from"
```
...
customerEmail -> emailFormat "validates"

# Validator validates entity
customerValidator -> customer "validates"
```
...
```dsl
# Entry point
customerController -> createCustomerCommand "creates"
customerController -> commandBus "dispatches via"

# Command handling
commandBus -> createCustomerHandler "routes to"
createCustomerHandler -> customer "creates"
createCustomerHandler -> customerRepository "uses"
...
customerCreatedEvent -> emailSubscriber "triggers"
customerCreatedEvent -> analyticsSubscriber "triggers"
customerCreatedEvent -> auditSubscriber "triggers"
...
✅ subscriber -> messageBroker "sends welcome email via"
...
handler -> validator "uses to validate customer email format"
```
...
createCustomerHandler -> customerRepository "uses"
updateCustomerHandler -> customerRepository "uses"
deleteCustomerHandler -> customerRepository "uses"
```
...
handler -> customer "creates"

# Handler publishes domain event
handler -> customerCreatedEvent "publishes"
```
...
customer -> customerRepositoryInterface "uses"
```
...
emailSubscriber -> customerCreatedEvent "listens to"
```
...
```dsl
componentA -> componentB "uses for customer validation"
```
