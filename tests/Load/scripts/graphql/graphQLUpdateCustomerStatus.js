...
export function setup() {
  ...
  if (response.status !== 200) {
    throw new Error(
      'Failed to fetch customer statuses for GraphQL update customer status load test.'
    );
  }
...
export default function updateCustomerStatus(data) {
  ...
  utils.checkResponse(response, 'customer status updated', res => {
    if (res.status !== 200) {
    ...
    if (!body.data.updateCustomerStatus.customerStatus) {
      console.error(
        `No customerStatus in updateCustomerStatus: ${JSON.stringify(body.data.updateCustomerStatus)}`
      );
      return false;
    }
