...
@test "worker memory verification uses a post-warmup baseline and only fails on sustained growth" {
  ...
  assert_output --partial 'warmup_iterations=${WORKER_MEMORY_WARMUP_ITERATIONS:-1}'
  assert_output --partial 'memory_sample_count=${WORKER_MEMORY_SAMPLE_COUNT:-5}'
  assert_output --partial 'run_soak_iteration "warmup ${warmup_iteration}/${warmup_iterations}"'
  assert_output --partial 'post_warmup_baseline_rss='
  assert_output --partial 'warmup_iterations='
  assert_output --partial 'mapfile -t sorted_samples'
  assert_output --partial 'memory_sample_count='
