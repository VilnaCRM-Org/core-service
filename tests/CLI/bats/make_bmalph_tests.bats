...
EOF
  ...
  if [ -f "${repo_root}/_bmad/COMMANDS.md" ]; then
    BMAD_TEST_CREATED_COMMANDS=0
    return 0
  fi

  mkdir -p "${repo_root}/_bmad"
  cat >"${repo_root}/_bmad/COMMANDS.md" <<'EOF'
# BMALPH Commands

> Test fixture generated by Bats. Do not edit.

## Agents

| Command | Description | Invocation |
|---------|-------------|------------|
| analyst | Research, briefs, discovery | Fixture wrapper command. |

## Phase 1: Analysis

| Command | Description | Invocation |
|---------|-------------|------------|
| create-brief | Create a product brief | Fixture wrapper command. |

## Phase 2: Planning

| Command | Description | Invocation |
|---------|-------------|------------|
| create-prd | Create a PRD | Fixture wrapper command. |

## Phase 3: Solutioning

| Command | Description | Invocation |
|---------|-------------|------------|
| create-architecture | Create architecture docs | Fixture wrapper command. |
| create-epics-stories | Create epics and stories | Fixture wrapper command. |
| implementation-readiness | Validate readiness | Fixture wrapper command. |

## bmalph CLI

| Command | Description | How to run |
|---------|-------------|------------|
| bmalph | BMAD master agent — navigate phases | Fixture wrapper command. |
EOF
  BMAD_TEST_CREATED_COMMANDS=1
}

teardown() {
  if [ "${BMAD_TEST_CREATED_COMMANDS:-0}" = "1" ] && [ -f "_bmad/COMMANDS.md" ]; then
    rm -f "_bmad/COMMANDS.md"
  fi
  ...
  unset BMAD_TEST_CREATED_COMMANDS BMAD_TEST_CREATED_CONFIG BMALPH_ORIGINAL_HOME BMALPH_ORIGINAL_HOME_SET BMALPH_TEST_HOME CS_USER_NPM_GLOBAL_BIN
}
